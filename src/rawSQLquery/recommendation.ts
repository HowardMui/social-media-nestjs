...
export const recommendUserList = ({
  ...
  return `
        ...
        from user as u
        left join (select userId, count(*) as 'postCount' from post group by userId) as up on u.userId = up.userId
        ...
        LEFT JOIN user_follows AS uf ON u.userId = uf.followingId AND uf.followerId = ${userId}
