...
export class RecommendationService {
  ...
  async getRecommendationUserList(query: RecommendationUserFilter) {
    try {
      const { limit, offset } = query;
  ...
  async getRecommendationTagList(query: RecommendationTagFilter) {
    try {
      const { limit, offset } = query;
      // * grt = get recommend tags
      const cacheRecommendationTag = await this.redis.getRedisValue<
      ...
      if (cacheRecommendationTag) {
        ...
        const formatTag = recommendedTag.map(({ _count, tagId, tagName }) => {
          return {
            tagId,
            tagName,
            postCount: _count.posts,
